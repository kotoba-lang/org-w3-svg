...
- `web/app.ts` builds SVGraph, browser-local coverage diagnostics, an SVGraph sidecar JSON, an SVGraph presentation package projection, canonical SVG source download, DrawingML fragment XML, DrawingML-to-SVG import for basic shape, common preset polygon/arc/flowchart/bevel/snip/symbol/star/arrow/callout/ribbon/action shape, custom geometry/freeform, grouped shape, connector, picture, and native table fragments, PresentationML slide XML, and a `.pptx` ZIP without Python or server APIs.
