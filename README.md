...
- Analysis: unsupported visual/layout attributes such as filters, masks, blend/isolation, fill/clip rules, paint order, unsupported vector effects, unsupported stroke line cap/join values, word spacing conflicts, dash offsets that cannot be approximated, multi-value text rotation, unsupported text/path length adjustments, unsupported text layout controls, unsupported markers, and unresolved paint servers are reported, while default no-op values and rendering quality hints are ignored
