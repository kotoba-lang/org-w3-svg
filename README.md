...
- SVG: `rect`, `circle`, `ellipse`, `line`, `polygon`, `polyline`, linear/quadratic/cubic/arc `path`, simple rect/text, line/text, and rect-background/line-border grids including merged cells and individual grid-line paint as native DrawingML tables, `text`, simple `tspan`, link/group containers, and basic `switch` fallback selection
