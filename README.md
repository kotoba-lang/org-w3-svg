...
- DrawingML: preset geometry shapes including common polygon, arrow, symbol, and flowchart presets, custom geometry paths including line/quadratic/cubic segments, native table cells, simple merged cells, and individual table cell borders under `p:graphicFrame` as editable SVG rectangles/lines/text, and text boxes under `p:sp`, with invalid numeric DrawingML attributes ignored where possible
