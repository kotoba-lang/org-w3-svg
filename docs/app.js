...
async function clearSavedSourceDocument() {
    const db = await openDocumentDb();
    await new Promise((resolve, reject) => {
        const transaction = db.transaction(documentStoreName, "readwrite");
...
export function buildSVGraph(svgText) {
...
export function buildSVGraphSidecar(svgraph, svgText = "") {
    return {
...
export function svgToPptx(svgText) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(svgText, "image/svg+xml");
    const error = doc.querySelector("parsererror");
    if (error)
...
export function drawingMlToSvg(drawingMlText) {
    const parser = new DOMParser();
...
function dmlPresetPoints(kind, box) {
    ...
    if (kind === "triangle" || kind === "flowChartExtract")
        return [[centerX, top], [right, bottom], [left, bottom]];
    if (kind === "flowChartMerge")
        return [[left, top], [right, top], [centerX, bottom]];
    if (kind === "rtTriangle")
        return [[left, top], [right, bottom], [left, bottom]];
    if (kind === "diamond" || kind === "flowChartDecision" || kind === "flowChartSort")
        return [[centerX, top], [right, centerY], [centerX, bottom], [left, centerY]];
    if (kind === "flowChartCollate")
        return [[left, top], [right, top], [centerX, centerY], [right, bottom], [left, bottom], [centerX, centerY]];
    if (kind === "parallelogram" || kind === "flowChartData" || kind === "flowChartInputOutput")
        return [[quarterX, top], [right, top], [threeQuarterX, bottom], [left, bottom]];
    if (kind === "trapezoid" || kind === "flowChartManualInput")
        return [[quarterX, top], [threeQuarterX, top], [right, bottom], [left, bottom]];
    if (kind === "nonIsoscelesTrapezoid")
        return [[box.x + box.width * 0.18, top], [right, top], [box.x + box.width * 0.82, bottom], [left, bottom]];
    if (kind === "flowChartManualOperation")
        return [[left, top], [right, top], [threeQuarterX, bottom], [quarterX, bottom]];
    if (kind === "flowChartDocument")
        return [[left, top], [right, top], [right, box.y + box.height * 0.82], [threeQuarterX, bottom], [centerX, box.y + box.height * 0.88], [quarterX, bottom], [left, box.y + box.height * 0.82]];
    if (kind === "flowChartPunchedCard")
        return [[box.x + box.width * 0.18, top], [right, top], [right, bottom], [left, bottom], [left, box.y + box.height * 0.18]];
    if (kind === "flowChartPunchedTape")
        return [[left, box.y + box.height * 0.12], [quarterX, top], [centerX, box.y + box.height * 0.12], [threeQuarterX, top], [right, box.y + box.height * 0.12], [right, box.y + box.height * 0.88], [threeQuarterX, bottom], [centerX, box.y + box.height * 0.88], [quarterX, bottom], [left, box.y + box.height * 0.88]];
    if (kind === "flowChartDelay")
        return [[left, top], [box.x + box.width * 0.7, top], [right, centerY], [box.x + box.width * 0.7, bottom], [left, bottom]];
    if (kind === "flowChartStoredData")
        return [[box.x + box.width * 0.15, top], [right, top], [right, bottom], [box.x + box.width * 0.15, bottom], [left, centerY]];
    if (kind === "flowChartDisplay")
        return [[left, top], [box.x + box.width * 0.8, top], [right, centerY], [box.x + box.width * 0.8, bottom], [left, bottom], [box.x + box.width * 0.15, centerY]];
    if (kind === "flowChartOffpageConnector")
        return [[left, top], [right, top], [right, threeQuarterY], [centerX, bottom], [left, threeQuarterY]];
    if (kind === "pentagon")
        return [[centerX, top], [right, box.y + box.height * 0.38], [box.x + box.width * 0.81, bottom], [box.x + box.width * 0.19, bottom], [left, box.y + box.height * 0.38]];
    if (kind === "hexagon" || kind === "flowChartPreparation")
        return [[quarterX, top], [threeQuarterX, top], [right, centerY], [threeQuarterX, bottom], [quarterX, bottom], [left, centerY]];
    if (kind === "heptagon")
    ...
    if (kind === "dodecagon" || kind === "flowChartOr" || kind === "flowChartSummingJunction")
        return regularPolygonPoints(12, box);
    if (kind === "bevel")
        return [[box.x + box.width * 0.18, top], [right - box.width * 0.18, top], [right, box.y + box.height * 0.18], [right, bottom - box.height * 0.18], [right - box.width * 0.18, bottom], [box.x + box.width * 0.18, bottom], [left, bottom - box.height * 0.18], [left, box.y + box.height * 0.18]];
    if (kind === "snip1Rect")
        return [[left, top], [threeQuarterX, top], [right, quarterY], [right, bottom], [left, bottom]];
    if (kind === "snip2SameRect")
        return [[quarterX, top], [right, top], [right, threeQuarterY], [threeQuarterX, bottom], [left, bottom], [left, quarterY]];
    if (kind === "snip2DiagRect")
        return [[left, top], [threeQuarterX, top], [right, quarterY], [right, bottom], [quarterX, bottom], [left, threeQuarterY]];
    if (kind === "chevron")
        return [[left, top], [threeQuarterX, top], [right, centerY], [threeQuarterX, bottom], [left, bottom], [quarterX, centerY]];
    if (kind === "homePlate")
        return [[left, top], [threeQuarterX, top], [right, centerY], [threeQuarterX, bottom], [left, bottom]];
    if (kind === "foldedCorner")
        return [[left, top], [threeQuarterX, top], [right, quarterY], [right, bottom], [left, bottom]];
    if (kind === "corner" || kind === "halfFrame")
        return [[left, top], [right, top], [right, quarterY], [quarterX, quarterY], [quarterX, bottom], [left, bottom]];
    if (kind === "diagStripe")
    ...
    if (kind === "rightBracket")
        return [[left, top], [right, top], [right, bottom], [left, bottom], [left, threeQuarterY], [threeQuarterX, threeQuarterY], [threeQuarterX, quarterY], [left, quarterY]];
    if (kind === "leftBrace")
        return [[right, top], [centerX, top], [quarterX, quarterY], [centerX, centerY], [quarterX, threeQuarterY], [centerX, bottom], [right, bottom], [threeQuarterX, threeQuarterY], [right, centerY], [threeQuarterX, quarterY]];
    if (kind === "rightBrace")
        return [[left, top], [centerX, top], [threeQuarterX, quarterY], [centerX, centerY], [threeQuarterX, threeQuarterY], [centerX, bottom], [left, bottom], [quarterX, threeQuarterY], [left, centerY], [quarterX, quarterY]];
    if (kind === "plus" || kind === "mathPlus")
        return [[box.x + box.width * 0.35, top], [box.x + box.width * 0.65, top], [box.x + box.width * 0.65, box.y + box.height * 0.35], [right, box.y + box.height * 0.35], [right, box.y + box.height * 0.65], [box.x + box.width * 0.65, box.y + box.height * 0.65], [box.x + box.width * 0.65, bottom], [box.x + box.width * 0.35, bottom], [box.x + box.width * 0.35, box.y + box.height * 0.65], [left, box.y + box.height * 0.65], [left, box.y + box.height * 0.35], [box.x + box.width * 0.35, box.y + box.height * 0.35]];
    if (kind === "mathMinus")
        return [[left, arrowShaftTop], [right, arrowShaftTop], [right, arrowShaftBottom], [left, arrowShaftBottom]];
    if (kind === "mathMultiply")
        return [[box.x + box.width * 0.2, top], [centerX, box.y + box.height * 0.3], [box.x + box.width * 0.8, top], [right, box.y + box.height * 0.2], [box.x + box.width * 0.7, centerY], [right, box.y + box.height * 0.8], [box.x + box.width * 0.8, bottom], [centerX, box.y + box.height * 0.7], [box.x + box.width * 0.2, bottom], [left, box.y + box.height * 0.8], [box.x + box.width * 0.3, centerY], [left, box.y + box.height * 0.2]];
    if (kind === "heart")
        return [[centerX, bottom], [left, box.y + box.height * 0.45], [box.x + box.width * 0.08, box.y + box.height * 0.18], [box.x + box.width * 0.32, top], [centerX, box.y + box.height * 0.2], [box.x + box.width * 0.68, top], [box.x + box.width * 0.92, box.y + box.height * 0.18], [right, box.y + box.height * 0.45]];
    ...
    if (kind === "star4")
        return [[centerX, top], [box.x + box.width * 0.6, box.y + box.height * 0.4], [right, centerY], [box.x + box.width * 0.6, box.y + box.height * 0.6], [centerX, bottom], [box.x + box.width * 0.4, box.y + box.height * 0.6], [left, centerY], [box.x + box.width * 0.4, box.y + box.height * 0.4]];
    if (kind === "star5")
        return [[centerX, top], [box.x + box.width * 0.62, box.y + box.height * 0.38], [right, box.y + box.height * 0.38], [box.x + box.width * 0.69, box.y + box.height * 0.59], [box.x + box.width * 0.81, bottom], [centerX, box.y + box.height * 0.72], [box.x + box.width * 0.19, bottom], [box.x + box.width * 0.31, box.y + box.height * 0.59], [left, box.y + box.height * 0.38], [box.x + box.width * 0.38, box.y + box.height * 0.38]];
    if (kind === "star6")
    ...
    if (kind === "star16")
        return regularStarPoints(16, box);
    if (kind === "sun")
        return regularStarPoints(16, box, 0.72);
    if (kind === "irregularSeal1")
        return regularStarPoints(16, box, 0.62);
    if (kind === "irregularSeal2")
        return regularStarPoints(24, box, 0.68);
    if (kind === "rightArrow")
        return [[left, quarterY], [arrowHeadX, quarterY], [arrowHeadX, top], [right, centerY], [arrowHeadX, bottom], [arrowHeadX, threeQuarterY], [left, threeQuarterY]];
    if (kind === "notchedRightArrow")
        return [[left, quarterY], [arrowHeadX, quarterY], [arrowHeadX, top], [right, centerY], [arrowHeadX, bottom], [arrowHeadX, threeQuarterY], [left, threeQuarterY], [quarterX, centerY]];
    if (kind === "leftArrow")
        return [[right, quarterY], [box.x + box.width * 0.35, quarterY], [box.x + box.width * 0.35, top], [left, centerY], [box.x + box.width * 0.35, bottom], [box.x + box.width * 0.35, threeQuarterY], [right, threeQuarterY]];
    if (kind === "upArrow")
        return [[quarterX, bottom], [quarterX, box.y + box.height * 0.35], [left, box.y + box.height * 0.35], [centerX, top], [right, box.y + box.height * 0.35], [threeQuarterX, box.y + box.height * 0.35], [threeQuarterX, bottom]];
    if (kind === "downArrow")
        return [[quarterX, top], [quarterX, arrowHeadY], [left, arrowHeadY], [centerX, bottom], [right, arrowHeadY], [threeQuarterX, arrowHeadY], [threeQuarterX, top]];
    ...
    directChildrenByLocal(tbl, "tr").forEach((row, rowIndex) => {
        ...
        for (const cell of directChildrenByLocal(row, "tc")) {
            ...
            for (let r = rowIndex; r < Math.min(rows.length, rowIndex + rowSpan); r += 1) {
                for (let c = colIndex; c < Math.min(columns.length, colIndex + colSpan); c += 1)
                    occupied[r][c] = true;
            }
...
function dmlTableCellBorderStyle(tcPr, name) {
    const line = childByLocal(tcPr, name);
    if (line && childByLocal(line, "noFill"))
        return null;
...
function dmlColor(parent) {
    ...
    if (srgb?.getAttribute("val"))
        return dmlApplyLuminanceModifiers(dmlHexColor(srgb.getAttribute("val")), srgb);
    const scrgb = childByLocal(parent, "scrgbClr");
    if (scrgb)
        return dmlScrgbColor(scrgb);
    const hsl = childByLocal(parent, "hslClr");
    if (hsl)
        return dmlHslColor(hsl);
    const scheme = childByLocal(parent, "schemeClr");
    if (scheme?.getAttribute("val"))
        return dmlSchemeColor(scheme);
    const system = childByLocal(parent, "sysClr");
    if (system?.getAttribute("lastClr"))
        return dmlApplyLuminanceModifiers(dmlHexColor(system.getAttribute("lastClr")), system);
    const preset = childByLocal(parent, "prstClr");
    if (preset?.getAttribute("val"))
        return dmlPresetColor(preset);
    return null;
}
function dmlScrgbColor(element) {
    if (element.getAttribute("r") == null || element.getAttribute("g") == null || element.getAttribute("b") == null)
        return null;
...
function dmlHslColor(element) {
    if (element.getAttribute("hue") == null || element.getAttribute("sat") == null || element.getAttribute("lum") == null)
        return null;
...
function dmlSchemeColor(element) {
    const colors = {
        accent1: "#4472c4",
        accent2: "#ed7d31",
        accent3: "#a5a5a5",
...
function dmlPresetColor(element) {
    const colors = {
        dkBlue: "#00008b",
        dkCyan: "#008b8b",
        dkGoldenrod: "#b8860b",
...
function dmlHexColor(value) {
    return value && /^[0-9a-f]{6}$/i.test(value) ? `#${value.toLowerCase()}` : null;
}
function dmlApplyLuminanceModifiers(color, element) {
    ...
    if (lumOff?.getAttribute("val") != null) {
        ...
        rgb = rgb.map((channel) => dmlRound(channel + (255 - channel) * offset));
    }
    return rgbToHex(rgb);
}
function dmlRound(value) {
...
function dmlPercentage(value, defaultValue) {
...
function dmlInt(value, defaultValue) {
    if (value == null)
        return defaultValue;
...
function dmlFloat(value, defaultValue) {
    if (value == null)
        return defaultValue;
    if (value.trim() === "")
        return defaultValue;
...
function dmlTextInsets(element) {
...
function dmlTextLayoutFontSize(runs) {
...
function dmlTextAnchor(element) {
    ...
    return { ctr: "middle", r: "end", l: "start" }[pPr?.getAttribute("algn") || ""] ?? null;
}
function dmlTextDirection(element) {
...
function dmlBaselineShift(value) {
    if (value >= 30000)
        return "super";
    if (value <= -25000)
        return "sub";
...
function coverageElementIsSupported(element, tag, refs, css = [], style = {}, viewport = defaultViewport()) {
    if (tag === "path")
...
function coverageReferencedSubtreeIsSupported(element, inheritedStyle, refs, css, viewport, refStack) {
    ...
    if (coverageElementIsIgnored(element, tag, style, refs, css, viewport, coverageIgnoredElements.has(tag)))
        return true;
    if (tag === "use")
        return coverageUseReferenceIsSupported(element, style, refs, css, viewport, refStack);
    if (tag === "path" && parseBasicPath(element.getAttribute("d") || "", [1, 0, 0, 1, 0, 0]) == null)
        return false;
...
function inspectCoveragePaintServers(element, style, tag, stats, refs, css) {
    ...
    for (const attr of ["fill", "stroke"]) {
        ...
        if (serverTag === "pattern") {
            if (!paintServerColor(ref.id, refs, style, new Set(), css))
...
function inspectCoverageReferencedSubtree(element, inheritedStyle, stats, refs, css, viewport, refStack) {
    ...
    if (style.display === "none")
        return;
    if (coverageHasNonRenderingGeometry(element, tag, style, css, viewport))
        return;
    if (coverageHasNoVisiblePaint(element, tag, style, refs, css, viewport, refStack) && !coverageHasUnresolvedPaintServer(style, refs, css))
        return;
...
function coverageHasUnresolvedPaintServer(style, refs, css) {
    for (const attr of ["fill", "stroke"]) {
        const value = style[attr];
        if (!value)
            continue;
        const ref = paintUrlRef(value);
        if (ref && !ref.fallback && !paintServerColor(ref.id, refs, style, new Set(), css))
            return true;
    }
    return false;
}
...
function coverageAttributeIsSupportedOrNoop(element, tag, name, value, style, refs, css, viewport) {
    ...
    if (["clip-path", "filter", "isolation", "mask", "mix-blend-mode"].includes(name) && !coverageSubtreeHasVisibleRendering(element, style, refs, css, viewport, new Set()))
        return true;
    if (name === "clip-path")
        return clipPathIsSupportedOrNoop(element, tag, value, style, refs, css, viewport);
    if (name === "clip-rule")
        return clipRuleHasNoEffect(element);
    if (name === "fill-rule")
    ...
    if (name === "dominant-baseline" || name === "alignment-baseline")
        return ["auto", "baseline", "alphabetic"].includes(normalized) || (tag === "text" && normalizeTextBaseline(value) != null) || firstPositionedTspanBaselineIsSupported(element, name, value);
...
function coverageAttributeHasNoEffect(element, name, value) {
    ...
    if (["color-rendering", "image-rendering", "shape-rendering", "text-rendering"].includes(name))
        return renderingQualityHintHasNoEffect(normalized);
...
function inspectCoverageTspanRunAttributes(element, style, tag, stats, refs, css, viewport) {
    if (tag !== "tspan")
        return;
    if (!tspanPositionIsSupportedOrNoop(element, style, refs, css, viewport)) {
        for (const attr of ["x", "y", "dx", "dy"]) {
            const value = element.getAttribute(attr);
            if (value != null && !tspanPositionAttrHasNoEffect(attr, value, viewport))
                addCoverageCount(stats.unsupported_attributes, attr);
        }
    }
    const declarations = resolvedCascadedDeclarations(element, css, style);
...
function tspanPositionIsSupportedOrNoop(element, style, refs, css, viewport) {
    if (!["x", "y", "dx", "dy"].some((attr) => element.hasAttribute(attr)))
        return true;
    if (!subtreeHasVisibleText(element, style, css, refs, viewport))
        return true;
    if (["x", "y", "dx", "dy"].every((attr) => !element.hasAttribute(attr) || tspanPositionAttrHasNoEffect(attr, element.getAttribute(attr) || "", viewport)))
        return true;
    return firstPositionedTspanPositionIsSupported(element) || lineBreakTspanPositionIsSupported(element);
}
function firstPositionedTspanPositionIsSupported(element) {
    const parent = element.parentElement;
    if (!parent || localName(parent) !== "text")
        return false;
    if (parent.hasAttribute("x") || parent.hasAttribute("y"))
        return false;
    if (parent.childNodes[0]?.nodeType === Node.TEXT_NODE && (parent.childNodes[0].textContent || "").trim())
        return false;
    if (!element.hasAttribute("x") || !element.hasAttribute("y"))
        return false;
    return !previousTextTspanHasContent(element);
}
function lineBreakTspanPositionIsSupported(element) {
    return element.hasAttribute("x") && element.hasAttribute("dy") && !element.hasAttribute("y") && !element.hasAttribute("dx");
}
function tspanPositionAttrHasNoEffect(attr, value, viewport) {
    if (attr === "x" || attr === "y")
        return false;
...
function firstPositionedTspanTextAnchorIsSupported(element, value) {
    if (localName(element) !== "tspan" || normalizeTextAnchor(value) == null)
        return false;
    return firstPositionedTspanPositionIsSupported(element);
}
function tspanTextAnchorHasNoEffect(element, value) {
    if (localName(element) !== "tspan")
        return false;
    const normalized = value.trim().toLowerCase();
    return ["", "start", "middle", "end"].includes(normalized) && !element.hasAttribute("x") && !element.hasAttribute("y");
}
function firstPositionedTspanBaselineIsSupported(element, name, value) {
    if (localName(element) !== "tspan" || (name !== "dominant-baseline" && name !== "alignment-baseline"))
        return false;
    if (normalizeTextBaseline(value) == null)
        return false;
    if (!firstPositionedTspanPositionIsSupported(element))
        return false;
    return true;
}
function previousTextTspanHasContent(element) {
    for (let sibling = element.previousElementSibling; sibling; sibling = sibling.previousElementSibling) {
        if (localName(sibling) === "tspan" && (sibling.textContent || "").trim())
            return true;
    }
    return false;
}
...
function clipRuleHasNoEffect(element) {
    for (let current = element.parentElement; current; current = current.parentElement) {
        if (localName(current) === "clipPath")
            return false;
    }
    return true;
}
...
function paintOrderHasNoEffect(tag, value, style) {
    ...
    if (["normal", "fill", "fill stroke", "fill stroke markers"].includes(normalized))
        return true;
    if (!["circle", "ellipse", "line", "path", "polygon", "polyline", "rect", "text", "tspan"].includes(tag))
        return false;
    const hasFill = tag !== "line" && style.fill !== "none" && style.fillAlpha !== 0;
    ...
    if (!(hasFill && hasStroke))
        return true;
    if (normalized === "markers fill stroke" || normalized === "fill markers stroke")
        return !hasVisibleMarker(style);
    return false;
}
...
function clipPathIsSupportedOrNoop(element, tag, value, style, refs, css, viewport) {
    if (clipPathTargetIsSupported(element, tag, value, refs))
        return true;
    if (!["a", "g", "svg", "switch", "symbol", "use"].includes(tag))
        return false;
    return subtreeClipPathIsSupported(element, value, style, refs, css, viewport, new Set());
}
function clipPathTargetIsSupported(element, tag, value, refs) {
    if (!clipPathHasRect(value, refs))
        return false;
    if (["rect", "circle", "ellipse", "line", "text", "image"].includes(tag))
        return true;
    if (tag === "polyline")
        return parsePoints(element.getAttribute("points") || "").length === 2;
    if (tag === "path") {
...
function subtreeClipPathIsSupported(element, inheritedClipPath, inheritedStyle, refs, css, viewport, refStack) {
    const tag = localName(element);
    const style = computedStyle(element, inheritedStyle, css, refs, viewport);
    if (style.display === "none")
        return true;
...
function elementToShape(element, matrix, style, id, viewport, css = [], refs = new Map()) {
    const tag = localName(element);
...
function extractSvgLineTable(shapes) {
    if (shapes.some((shape) => !["line", "text"].includes(shape.kind)))
        return null;
    const lines = shapes.filter((shape) => shape.kind === "line" && svgTableLineCandidate(shape));
    if (lines.length < 4 || lines.length !== shapes.filter((shape) => shape.kind === "line").length)
        return null;
    const verticals = lines.filter(svgTableVerticalLine);
    const horizontals = lines.filter(svgTableHorizontalLine);
    if (verticals.length < 3 || horizontals.length < 3 || verticals.length + horizontals.length !== lines.length)
        return null;
    const xEdges = tableEdges(verticals.map(lineMinX));
    const yEdges = tableEdges(horizontals.map(lineMinY));
    if (xEdges.length < 3 || yEdges.length < 3)
        return null;
    const xMin = xEdges[0];
    const xMax = xEdges[xEdges.length - 1];
    const yMin = yEdges[0];
    const yMax = yEdges[yEdges.length - 1];
    if (!tableLinesCoverEdges(verticals, xEdges, yMin, yMax, "vertical"))
        return null;
    if (!tableLinesCoverEdges(horizontals, yEdges, xMin, xMax, "horizontal"))
        return null;
    const columns = xEdges.slice(1).map((edge, index) => edge - xEdges[index]);
    const rows = yEdges.slice(1).map((edge, index) => edge - yEdges[index]);
    if (columns.some((width) => width <= 0) || rows.some((height) => height <= 0))
...
function tableCellFromRect(rect, text, row, col, colSpan, rowSpan, borders) {
...
function rectGridLines(shapes, xEdges, yEdges, origins) {
    if (origins.size !== (xEdges.length - 1) * (yEdges.length - 1))
        return null;
    if ([...origins.values()].some(({ colSpan, rowSpan }) => colSpan !== 1 || rowSpan !== 1))
        return null;
    const lines = shapes.filter((shape) => shape.kind === "line");
    if (!lines.length || lines.some((line) => !svgTableLineCandidate(line)))
        return null;
    const verticals = lines.filter(svgTableVerticalLine);
    const horizontals = lines.filter(svgTableHorizontalLine);
    if (verticals.length + horizontals.length !== lines.length)
        return null;
    if (!tableLinesCoverEdges(verticals, xEdges, yEdges[0], yEdges[yEdges.length - 1], "vertical"))
        return null;
    if (!tableLinesCoverEdges(horizontals, yEdges, xEdges[0], xEdges[xEdges.length - 1], "horizontal"))
        return null;
    return lines;
}
function svgTableLineCandidate(shape) {
    return Boolean(shape.stroke) && (svgTableVerticalLine(shape) || svgTableHorizontalLine(shape));
}
function svgTableVerticalLine(shape) {
    return numbersClose(shape.x1, shape.x2, 1e-6) && Math.abs(shape.y2 - shape.y1) > 0;
}
function svgTableHorizontalLine(shape) {
    return numbersClose(shape.y1, shape.y2, 1e-6) && Math.abs(shape.x2 - shape.x1) > 0;
}
function tableLinesCoverEdges(lines, edges, start, end, orientation) {
    return edges.every((edge) => lines.some((line) => tableLineCoversEdge(line, edge, start, end, orientation)));
}
function tableLineCoversEdge(line, edge, start, end, orientation) {
    if (orientation === "vertical")
        return numbersClose(lineMinX(line), edge, 1e-6) && numbersClose(lineMinY(line), start, 1e-6) && numbersClose(lineMaxY(line), end, 1e-6);
    return numbersClose(lineMinY(line), edge, 1e-6) && numbersClose(lineMinX(line), start, 1e-6) && numbersClose(lineMaxX(line), end, 1e-6);
}
function tableGridBorders(lines, xEdges, yEdges) {
    ...
    for (let index = 0; index < xEdges.length; index += 1)
        if (!verticals.has(index))
            return null;
    for (let index = 0; index < yEdges.length; index += 1)
        if (!horizontals.has(index))
            return null;
    return [verticals, horizontals];
}
function cellGridBorders(borders, row, col) {
...
function tableBorderFromLine(line) {
    return {
        stroke: line.stroke,
...
function tableGridPaint(lines) {
    return tableBorderFromLine(lines.find((line) => line.stroke) ?? lines[0]);
}
function syntheticTableRect(x, y, width, height, border) {
    return {
        id: 0,
        kind: "rect",
        name: "table-cell",
...
function lineMinX(line) { return Math.min(line.x1, line.x2); }
function lineMaxX(line) { return Math.max(line.x1, line.x2); }
function lineMinY(line) { return Math.min(line.y1, line.y2); }
function lineMaxY(line) { return Math.max(line.y1, line.y2); }
...
function htmlTableFitSpacing(spacing, size) {
    if (spacing <= 0 || size <= 0)
        return 0;
...
function htmlTableFirstRowColumnWidths(table, count, width) {
    const specs = [];
...
function htmlTableSizes(specs, count, size) {
    if (count <= 0)
        return [];
    const normalized = specs.slice(0, count);
    while (normalized.length < count)
        normalized.push(null);
    const fixedTotal = normalized.reduce((sum, item) => sum + (item ?? 0), 0);
    const missing = normalized.filter((item) => item == null).length;
    if (fixedTotal <= 0)
        return Array.from({ length: count }, () => Math.max(1, size / count));
    if (missing) {
...
function htmlTableColumnBackgrounds(table, count, css) {
    ...
    for (const child of Array.from(table.children)) {
        ...
        const cols = tag === "colgroup"
            ...
            : tag === "col" ? [child] : [];
        for (const col of cols) {
            const layers = [htmlElementBackgroundFill(col, css), colgroupFill].filter((item) => item != null);
            const span = Math.max(1, htmlSpan(col, "span"));
            for (let index = 0; index < span; index += 1)
                backgrounds.push(layers);
    ...
    while (backgrounds.length < count)
        backgrounds.push([]);
    return backgrounds.slice(0, count);
}
function htmlTableCellFill(cell, table, columnBackgrounds, css) {
...
function findLastElement(items, predicate) {
...
function htmlAncestorsBetween(root, element) {
    const ancestors = [];
...
function htmlElementStyle(element, inheritedStyle, css) {
    ...
    if (letterSpacing != null)
        next.letterSpacing = normalizeSpacingLength(letterSpacing, next.fontSize ?? 14);
    if (direction != null)
...
function htmlFirstColorFill(value, style) {
...
function textRuns(element, inheritedStyle, viewport = defaultViewport(), metricScale = 1, css = [], refs = new Map()) {
    const runs = [];
...
function svgTextPosition(element, viewport, css = [], inheritedStyle = {}) {
    const declarations = resolvedCascadedDeclarations(element, css, inheritedStyle);
...
function firstPositionedTspanAnchor(element, inheritedStyle, css = [], refs = new Map(), viewport = defaultViewport()) {
...
function firstPositionedTspanBaseline(element, inheritedStyle, css = [], refs = new Map(), viewport = defaultViewport()) {
    if (element.hasAttribute("x") || element.hasAttribute("y") || (element.childNodes[0]?.nodeType === Node.TEXT_NODE && (element.childNodes[0].textContent || "").trim()))
        return null;
    const first = firstTextTspan(element);
    if (!first || !first.hasAttribute("x") || !first.hasAttribute("y"))
...
function tspanStartsNewLine(tspan, viewport, css = [], inheritedStyle = {}) {
    const declarations = resolvedCascadedDeclarations(tspan, css, inheritedStyle);
    if (cascadedGeomValue(tspan, declarations, "x") != null || cascadedGeomValue(tspan, declarations, "y") != null)
        return true;
...
function textRotation(element, style, css = [], refs = new Map(), viewport = defaultViewport()) {
...
function spXml(id, name, x, y, width, height, prst, style, body, rotation = null, xfrmAttrs = "") {
...
function prepareSlideMedia(slideXml, firstMediaIndex, layoutIndex = 1) {
    const media = {};
...
export function initSVGraphEditor() {
    source = mustElement("source");
    preview = mustElement("preview");
    panel = mustElement("panel");
    fileInput = mustElement("fileInput");
    undoButton = mustElement("undoBtn");
    redoButton = mustElement("redoBtn");
    clearSavedButton = mustElement("clearSavedBtn");
    ...
    mustElement("downloadSidecarBtn").addEventListener("click", () => {
        if (state.svgraph)
            downloadText("svgraph-sidecar.json", JSON.stringify(buildSVGraphSidecar(state.svgraph, source.value), null, 2));
    });
...
if (typeof document !== "undefined" && document.getElementById("source"))
    initSVGraphEditor();
...
function cascadedGeomValue(element, declarations, name) {
    return declarations[name] ?? element.getAttribute(name);
}
function cascadedGeom(element, declarations, name, axis, viewport, fallback = 0) {
    const value = cascadedGeomValue(element, declarations, name);
    return parseCssLength(value, percentageBasis(axis, viewport), fallback);
}
function optionalCascadedGeom(element, declarations, name, axis, viewport) {
    const value = cascadedGeomValue(element, declarations, name);
    if (value == null)
        return null;
...
function computedStyle(element, inherited, css = [], refs = new Map(), viewport = defaultViewport()) {
    ...
    if (strokeDasharray != null)
        next.strokeDasharray = normalizeStrokeDasharray(strokeDasharray, dashBasis);
    ...
    if (markerStart != null)
        next.markerStart = normalizeMarkerReference(markerStart, refs);
    if (markerMid != null) {
        next.markerMid = normalizeMarkerReference(markerMid, refs);
        next.markerMidSource = next.markerMid ? "marker-mid" : null;
    }
    if (markerEnd != null)
        next.markerEnd = normalizeMarkerReference(markerEnd, refs);
    return next;
}
...
function scaledTextMetricsStyle(style, scale) {
    if (Math.abs(scale - 1) < 1e-9)
        return style;
    const next = { ...style };
    if (next.fontSize != null)
        next.fontSize *= scale;
    if (next.textLength != null)
        next.textLength *= scale;
    if (next.letterSpacing != null)
        next.letterSpacing *= scale;
    if (next.wordSpacing != null)
        next.wordSpacing *= scale;
    if (next.textDecorationThickness != null)
        next.textDecorationThickness *= scale;
    return next;
}
function scaledPathLengthDashStyle(style, scale) {
    if (Math.abs(scale - 1) < 1e-9 || !style.strokeDasharray)
        return style;
    const next = { ...style };
...
function pathActualLength(element, tag, viewport, points, css = [], inheritedStyle = {}) {
    ...
    if (points && points.length >= 2)
        return polylineLength(points, tag === "polygon");
    return null;
}
...
function normalizeVisibility(value) {
    const normalized = value.trim().toLowerCase().split(/\s+/).join(" ");
    if (normalized === "hidden" || normalized === "collapse")
        return normalized;
...
function normalizeStrokeDasharrayWithOffset(value, offset, basis = rootFontSize) {
    const nums = dasharrayWithOffset(value, offset, basis);
    return nums ? nums.map(formatNumber).join(" ") : null;
}
function normalizeSpacingLength(value, basis = rootFontSize) {
    const normalized = value.trim().toLowerCase();
...
function dasharrayNumbers(value, basis = rootFontSize) {
...
function dasharrayWithOffset(value, offset, basis = rootFontSize) {
    let nums = dasharrayNumbers(value, basis);
    if (!nums?.length || nums.reduce((sum, item) => sum + item, 0) <= 0)
        return null;
    if (nums.length % 2 === 1)
    ...
    for (let index = 0; index < nums.length; index += 1) {
        ...
        if (phase < end || numbersClose(phase, end)) {
            ...
            if (index % 2 === 1) {
                ...
                if (!numbersClose(consumedGap, 0))
                    shifted.push(consumedGap);
                if (shifted.length % 2 === 1)
                    shifted.push(0);
                return shifted;
            }
            ...
            if (!numbersClose(consumed, 0))
                shifted.push(consumed);
            if (shifted.length % 2 === 1)
                shifted.push(0);
            return shifted;
        }
...
function numbersClose(left, right, tolerance = 1e-9) {
    return Math.abs(left - right) < tolerance;
}
function normalizePaint(value, refs = new Map(), style = {}, css = []) {
    return normalizePaintValue(value, refs, style, css)?.color ?? null;
}
function normalizePaintValue(value, refs = new Map(), style = {}, css = []) {
...
function contextPaintValue(value, style) {
    const normalized = value.trim().toLowerCase();
    if (normalized === "context-fill" && style.fill)
        return { color: style.fill, alpha: style.fillAlpha ?? cssColorAlpha(style.fill) };
    if (normalized === "context-stroke" && style.stroke)
        return { color: style.stroke, alpha: style.strokeAlpha ?? cssColorAlpha(style.stroke) };
    return null;
}
function paintServerColor(id, refs, style, seen = new Set(), css = []) {
    if (seen.has(id))
        return null;
    ...
    const tag = localName(element);
    if (tag === "pattern")
        return averageColor(patternColors(element, refs, {}, new Set([...seen, id]), css));
    if (tag !== "linearGradient" && tag !== "radialGradient")
        return null;
...
function patternColors(element, refs, inheritedStyle, seen, css = []) {
    const colors = [];
    for (const child of Array.from(element.children)) {
        const tag = localName(child);
        ...
        if (!["rect", "circle", "ellipse", "path", "polygon", "polyline", "text", "tspan", "line"].includes(tag))
            continue;
        if (tag !== "line" && style.fill && style.fillAlpha !== 0)
            colors.push(style.fill);
        if (style.stroke && style.strokeAlpha !== 0)
            colors.push(style.stroke);
        colors.push(...patternColors(child, refs, style, seen, css));
    }
    return colors;
}
function simpleElementStyle(element, inheritedStyle, refs, seen, css = []) {
    const declarations = resolvedCascadedDeclarations(element, css, inheritedStyle);
    ...
    if (visibility != null)
        next.visibility = normalizeVisibility(visibility);
    if (fill != null)
        next.fill = normalizePatternPaint(fill, refs, next, seen, css);
    if (stroke != null)
        next.stroke = normalizePatternPaint(stroke, refs, next, seen, css);
    if (fill != null || opacityAlpha != null || fillOpacity != null)
        next.fillAlpha = combinedAlpha(opacityAlpha, parseAlpha(fillOpacity), next.fillAlpha);
    if (stroke != null || opacityAlpha != null || strokeOpacity != null)
        next.strokeAlpha = combinedAlpha(opacityAlpha, parseAlpha(strokeOpacity), next.strokeAlpha);
    return next;
}
function normalizePatternPaint(value, refs, style, seen, css = []) {
    const trimmed = value.trim();
    if (!trimmed || trimmed === "none" || trimmed === "transparent")
        return null;
    const ref = paintUrlRef(trimmed);
...
function gradientStops(element, refs, style, seen, css = []) {
    if (!element)
        return [];
    const tag = localName(element);
    if (tag !== "linearGradient" && tag !== "radialGradient")
    ...
    if (href.startsWith("#")) {
        ...
        if (inherited && inheritedId && !seen.has(inheritedId))
            colors.push(...gradientStops(inherited, refs, gradientStyle, new Set([...seen, inheritedId]), css));
    }
...
function parseFontSize(value, inherited = rootFontSize) {
    if (!value)
        return inherited;
    const normalized = value.trim().toLowerCase();
    const keywords = {
...
function calcLength(body, basis) {
...
function splitCalcAddends(value) {
    const result = [];
    let current = "";
    let sign = 1;
    let depth = 0;
...
function calcProduct(value, basis) {
...
function splitCalcFactors(value) {
    const result = [];
    let current = "";
    let operator = "*";
    let depth = 0;
    for (const char of value) {
        ...
        if ((char === "*" || char === "/") && depth === 0) {
            if (current.trim())
                result.push([operator, current.trim()]);
            operator = char;
            current = "";
            continue;
        }
...
function factorLength(value, basis) {
    const trimmed = stripOuterParens(value.trim());
    if (trimmed.toLowerCase().startsWith("calc(") && trimmed.endsWith(")"))
        return calcLength(trimmed.slice(5, -1), basis);
    if (splitCalcAddends(trimmed).length > 1)
        return calcLength(trimmed, basis);
    return parseCssLength(trimmed, basis, Number.NaN);
}
function factorNumber(value, basis) {
    const trimmed = stripOuterParens(value.trim());
    const parsed = Number.parseFloat(trimmed);
    if (Number.isFinite(parsed) && !/[a-z%]/i.test(trimmed))
        return parsed;
    return factorLength(trimmed, basis);
}
function factorDivisor(value, basis) {
    const parsed = factorNumber(value, basis);
    return parsed == null || Math.abs(parsed) < 0.000001 ? null : parsed;
}
function cssLengthFunction(value, basis) {
    const nameEnd = value.indexOf("(");
    if (nameEnd <= 0 || !value.endsWith(")"))
        return null;
    ...
    if (args.some((item) => !Number.isFinite(item)))
        return null;
    if (name === "min")
...
function stripOuterParens(value) {
    ...
    while (trimmed.startsWith("(") && trimmed.endsWith(")")) {
        ...
        if (splitCssTopLevel(inner, ",").length !== 1)
            break;
        trimmed = inner.trim();
    }
    return trimmed;
}
function calcSignIsUnary(value, index) {
...
function imagePreserveAspectRatioRect(x, y, width, height, href, value) {
    if (!value)
        return { x, y, width, height, srcRect: null };
    ...
    if (meetOrSlice === "slice")
        return { x, y, width, height, srcRect: imageSliceSrcRect(intrinsic.width, intrinsic.height, width, height, align) };
...
function imageSliceSrcRect(imageWidth, imageHeight, viewportWidth, viewportHeight, align) {
    const imageAspect = imageWidth / imageHeight;
    const viewportAspect = viewportWidth / viewportHeight;
    if (numbersClose(imageAspect, viewportAspect))
        return null;
...
function alignedCrop(total, alignment) {
    if (alignment === "Min")
        return [0, total];
    if (alignment === "Max")
        return [total, 0];
...
function dataImageDimensions(uri) {
...
function matrixKeepsRectAxisAligned(matrix) {
    const [a, b, c, d] = matrix;
    const epsilon = 1e-9;
...
function presentationXml(slideCount, [width, height], masterCount = 1) {
...
function presentationRels(slideCount, masterCount = 1) {
...
function slideLayoutRel(layoutIndex = 1) {
...
function slideMasterRels(layoutIndex = 1) {
