...
- Aligned browser coverage diagnostics for inherited text layout attributes with the Python analyzer, ignoring text nodes that have no visible paint.
...
- Aligned browser `foreignObject` HTML table shorthand style parsing with Python for first background colors, parenthesized border colors, and `thin`/`medium`/`thick` border widths.
...
- Added `foreignObject` HTML table `text-transform` support to Python and browser rich text runs.
- Added `foreignObject` HTML table `word-spacing` support to Python and browser rich text runs using the existing DrawingML character-spacing approximation.
...
- Added browser TypeScript `drawingMlToSvg` import support for native DrawingML table fragments as semantic SVG table and cell nodes.
...
- Aligned browser TypeScript `drawingMlToSvg` primitive preset import mapping for DrawingML flowchart connector, terminator, and alternate-process shapes, including Python-compatible round-rect radius.
...
- Aligned browser `fill-rule` diagnostics with Python so invisible fill and inherited stroke-only subtrees are treated as no-ops.
- Aligned browser `stroke-dashoffset` diagnostics with Python for no-op dash phases, invalid dash arrays, and invisible strokes.
- Aligned browser `overflow` diagnostics with Python for visible/default values, non-SVG elements, and hidden SVGs without visible rendering.
