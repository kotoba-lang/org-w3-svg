...
def test_release_checklist_covers_distribution_and_pptx_smoke() -> None:
    ...
    assert "examples/coverage.svg -o tmp/svgraph-coverage.pptx" in release
    assert "python -m zipfile --test tmp/svgraph-coverage.pptx" in release
...
def test_foreign_object_html_table_text_transform_converts_to_native_text_runs() -> None:
    svg = """<svg width="150" height="50">
      <foreignObject x="10" y="8" width="120" height="24">
        <body xmlns="http://www.w3.org/1999/xhtml">
          <table>
            <tr>
              <td style="border:1px solid #94a3b8;text-transform:lowercase">LOUD <span style="text-transform:uppercase">quiet</span></td>
    ...
    assert "<a:t>QUIET</a:t>" in dml
    assert "<a:t>Static</a:t>" in dml
    assert "LOUD" not in dml
    assert "quiet" not in dml
    assert analyze_svg(svg).unsupported_elements == {}
...
def test_drawingml_invalid_numeric_text_run_values_fall_back_to_defaults() -> None:
    svg = drawingml_to_svg(Path("examples/text-invalid-numeric.dml").read_text(encoding="utf-8"))

    assert "Invalid numeric text" in svg
...
def test_drawingml_invalid_numeric_custom_dash_values_fall_back_to_defaults() -> None:
    svg = drawingml_to_svg(Path("examples/line-invalid-dash.dml").read_text(encoding="utf-8"))

    assert "<line" in svg
    assert 'stroke="#111111"' in svg
    assert 'stroke-width="2"' in svg
    assert 'stroke-dasharray="0 0"' in svg
    assert 'stroke-dasharray="4 2"' not in svg
    assert analyze_svg(svg).unsupported_attributes == {}


def test_drawingml_invalid_picture_alpha_values_fall_back_to_defaults() -> None:
    svg = drawingml_to_svg(Path("examples/picture-invalid-alpha.dml").read_text(encoding="utf-8"))

    assert "<image" in svg
    assert PNG_DATA_URI in svg
    assert "opacity" not in svg
    assert analyze_svg(svg).unsupported_attributes == {}


def test_drawingml_invalid_picture_source_crop_values_fall_back_to_defaults() -> None:
    svg = drawingml_to_svg(Path("examples/picture-invalid-src-rect.dml").read_text(encoding="utf-8"))
...
def test_drawingml_all_caps_text_run_round_trips_to_svg_all_small_caps_variant() -> None:
    svg = drawingml_to_svg(Path("examples/text-caps.dml").read_text(encoding="utf-8"))

    assert 'font-variant="all-small-caps"' in svg
    assert 'font-variant="small-caps"' not in svg
    assert "All Caps" in svg
    assert analyze_svg(svg).unsupported_attributes == {}
...
def test_drawingml_native_table_truthy_merge_attrs_are_continuations() -> None:
    dml = """<p:spTree xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"
      xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
      <p:graphicFrame>
        <p:nvGraphicFramePr><p:cNvPr id="1" name="bool merge"/><p:cNvGraphicFramePr/><p:nvPr/></p:nvGraphicFramePr>
        <p:xfrm><a:off x="0" y="0"/><a:ext cx="762000" cy="1143000"/></p:xfrm>
        <a:graphic><a:graphicData><a:tbl>
          <a:tblGrid><a:gridCol w="381000"/><a:gridCol w="381000"/></a:tblGrid>
          <a:tr h="381000">
            <a:tc gridSpan="2"><a:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr sz="1000"/><a:t>Wide</a:t></a:r></a:p></a:txBody><a:tcPr/></a:tc>
            <a:tc hMerge="true"><a:txBody><a:bodyPr/><a:lstStyle/><a:p/></a:txBody><a:tcPr/></a:tc>
          </a:tr>
          <a:tr h="381000">
            <a:tc rowSpan="2"><a:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr sz="1000"/><a:t>Tall</a:t></a:r></a:p></a:txBody><a:tcPr/></a:tc>
            <a:tc><a:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr sz="1000"/><a:t>B</a:t></a:r></a:p></a:txBody><a:tcPr/></a:tc>
          </a:tr>
          <a:tr h="381000">
            <a:tc vMerge="true"><a:txBody><a:bodyPr/><a:lstStyle/><a:p/></a:txBody><a:tcPr/></a:tc>
            <a:tc><a:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr sz="1000"/><a:t>D</a:t></a:r></a:p></a:txBody><a:tcPr/></a:tc>
          </a:tr>
        </a:tbl></a:graphicData></a:graphic>
      </p:graphicFrame>
    </p:spTree>"""
    ...
    assert '<rect fill="none" stroke="none" x="0" y="0" width="80" height="40"/>' in svg
    assert '<rect fill="none" stroke="none" x="0" y="40" width="40" height="80"/>' in svg
    assert '<rect fill="none" stroke="none" x="40" y="40" width="40" height="40"/>' in svg
    assert '<rect fill="none" stroke="none" x="40" y="80" width="40" height="40"/>' in svg
    assert svg.count("<rect") == 4
    assert "Wide" in svg
    assert "Tall" in svg
    assert "B" in svg
