...
def test_web_source_and_package_metadata_use_svgraph_naming() -> None:
    ...
    assert "examples/table-border-effects.dml" in package_metadata["files"]
    ...
    for generated in [source, app_js]:
        ...
        assert "function dmlSvgMarkerDefs" in generated
        assert "function dmlLineArrow" in generated
        ...
        assert 'dmlLineArrow(childByLocal(ln, "tailEnd"))' in generated
        assert 'dmlLineArrow(childByLocal(ln, "headEnd"))' in generated
        assert 'marker-start="url(#' in generated
        assert 'marker-end="url(#' in generated
        ...
        assert "function romanNumber" in generated
...
def test_browser_only_svgraph_build_is_documented_and_ci_guarded() -> None:
    ...
    assert "line arrowhead import as SVG markers" in readme
    ...
    assert 'id=\\\"boolean-merge-table\\\"' in package_metadata["scripts"]["check:package"]
    assert 'x=\\\"0\\\" y=\\\"0\\\" width=\\\"80\\\" height=\\\"40\\\"' in package_metadata["scripts"]["check:package"]
    ...
    assert 'id=\\\"boolean-flip-line\\\"' in package_metadata["scripts"]["check:package"]
    assert 'x1=\\\"50\\\"' in package_metadata["scripts"]["check:package"]
    assert 'y2=\\\"20\\\"' in package_metadata["scripts"]["check:package"]
    ...
    assert 'id=\\\"border-effect-table\\\"' in package_metadata["scripts"]["check:package"]
    ...
    assert 'stroke=\\\"#800080\\\"' in package_metadata["scripts"]["check:package"]
    assert 'stroke=\\\"#008000\\\"' in package_metadata["scripts"]["check:package"]
...
def test_changelog_documents_svgraph_migration_guard_surfaces() -> None:
    ...
    for expected in [
        ...
        "browser TypeScript `drawingMlToSvg` native table border gradient and pattern fill fallbacks",
